...
public class AuthService : IAuthService
{
    ...
    public AuthService(
        ...
        SignInManager<AuthAccount> signInManager,
    ...
    public async Task<Result<AuthResponseDto>> AuthenticateAsync(AuthDto dto, string deviceInfo)
    {
        ...
        if (!authResult.Succeeded)
        {
            ...
            return Result.BadRequest<AuthResponseDto>(_config["AccountErrors:InvalidCredentials"] ?? "");
        }
    ...
    public async Task<PaginatedResponse<UserSessionDto>> GetAllActiveSessionsByAccountIdAsync(Pagination parameters, string accountId)
    {
    ...
    private async Task<JwtSecurityToken> GenerateJwtToken(AuthAccount account)
    {
        ...
        var claims = new[]
        {
            ...
            new Claim("fullName", $"{account.Name} {account.LastName}".Trim())
    ...
    private async Task<string> GetResetPasswordUri(AuthAccount account, string origin)
    {
